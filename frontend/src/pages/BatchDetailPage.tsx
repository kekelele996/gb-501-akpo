...
import { effectiveSegment } from '../utils/segment'
...
export function BatchDetailPage() {
  ...
  return (
    <div className="page-stack">
      ...
      <section className="detail-section"><Typography.Title level={4}>检验明细</Typography.Title><EntityTable<InspectionSample> size="small" pagination={false} dataSource={batch.inspections || []} columns={[{ title: '样本', dataIndex: 'sampleCode' }, { title: '段位', render: (_, row) => { const segment = effectiveSegment(row); return segment ? <StatusBadge value={segment} /> : '-' } }, { title: '抽样位置', dataIndex: 'samplingPosition' }, { title: '检验项', dataIndex: 'inspectionItem' }, { title: '结果', dataIndex: 'result', render: (value) => <StatusBadge value={value} /> }, { title: '测量值', dataIndex: 'measuredValue' }, { title: '接受范围', dataIndex: 'acceptanceRange' }, { title: '复测', dataIndex: 'retestStatus', render: (value) => <StatusBadge value={value} /> }]} /></section>
      <Divider />
