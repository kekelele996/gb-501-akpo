...
import type { InspectionSample, ProductionBatch, SampleSegment } from '../types/domain'
import { formatDateTime } from '../utils/format'
import { SEGMENT_LABELS, SEGMENT_OPTIONS, effectiveSegment, occupiedSegments } from '../utils/segment'
...
export function InspectionsPage() {
  ...
  const selectedBatchId = Form.useWatch('productionBatchId', createForm)
  ...
  const refreshBatches = () => batchAPI.list({ page: 1, pageSize: 100 }).then((value) => setBatches(value.items.filter((batch) => ['running', 'hold', 'rework'].includes(batch.status))))
  ...
  const takenSegments = useMemo(() => new Set(occupiedSegments(selectedBatch?.inspections || [])), [selectedBatch])

  const create = async () => {
    const values = await createForm.validateFields()
    setSaving(true)
    try { await inspectionAPI.create({ ...values, notes: values.notes || '' }); message.success('检验样本已登记'); setCreateOpen(false); createForm.resetFields(); await Promise.all([refresh(), refreshBatches()]) } finally { setSaving(false) }
  }
  const complete = async () => {
    if (!completeTarget) return
    const values = await completeForm.validateFields()
    setSaving(true)
    try { await inspectionAPI.complete(completeTarget.id, values); message.success('检验结果已提交'); setCompleteTarget(null); completeForm.resetFields(); await Promise.all([refresh(), refreshBatches()]) } finally { setSaving(false) }
  }
  const columns: ColumnsType<InspectionSample> = [
    ...
    { title: '段位', render: (_, row) => { const segment = effectiveSegment(row); return segment ? <StatusBadge value={segment} /> : <Tag>未识别</Tag> } },
  ...
  return (
    <div className="page-stack">
      <header className="page-header"><div><Typography.Title level={2}>检验工作台</Typography.Title><Typography.Text type="secondary">按批次起始段、中段、末段登记抽样并录入检验结果</Typography.Text></div><Button type="primary" icon={<PlusOutlined />} disabled={!can('inspection:write')} onClick={() => setCreateOpen(true)}>登记样本</Button></header>
      ...
      <Modal title="登记检验样本" width={640} open={createOpen} confirmLoading={saving} onOk={() => void create()} onCancel={() => setCreateOpen(false)} okText="登记" cancelText="取消">
        <Form form={createForm} layout="vertical">
          <Form.Item name="productionBatchId" label="生产批次" rules={[{ required: true, message: '请选择生产批次' }]}><Select showSearch optionFilterProp="label" placeholder="选择批次后才能选择段位" onChange={() => { createForm.setFieldValue('segment', undefined); createForm.setFieldValue('samplingPosition', '') }} options={batches.map((batch) => ({ value: batch.id, label: `${batch.batchNo} · ${batch.specification}` }))} /></Form.Item>
          <Form.Item name="segment" label="段位" rules={[{ required: true, message: '只能选择批次起始段、中段或末段' }]}>
            <Radio.Group optionType="button" buttonStyle="solid" disabled={!selectedBatchId} onChange={(event) => { const segment = event.target.value as SampleSegment; if (!createForm.getFieldValue('samplingPosition')) createForm.setFieldValue('samplingPosition', SEGMENT_LABELS[segment]) }} options={SEGMENT_OPTIONS.map((option) => ({ ...option, disabled: takenSegments.has(option.value) }))} />
          </Form.Item>
          {selectedBatchId && takenSegments.size > 0 && <Typography.Text type="secondary">已占用段位不可重复登记，需待该段位样本不合格释放后方可补登。</Typography.Text>}
          <Row gutter={16}><Col span={12}><Form.Item name="sampleCode" label="样本编号" rules={[{ required: true, min: 3 }]}><Input /></Form.Item></Col><Col span={12}><Form.Item name="samplingPosition" label="抽样位置" rules={[{ required: true }]}><Input placeholder="起始/中段/末段具体位置" /></Form.Item></Col></Row>
          <Form.Item name="inspectionItem" label="检验项目" rules={[{ required: true }]}><Input placeholder="密封强度、染色渗透等" /></Form.Item>
          <Form.Item name="acceptanceRange" label="接受范围" rules={[{ required: true }]}><Input placeholder="≥ 1.5 N/15mm" /></Form.Item>
