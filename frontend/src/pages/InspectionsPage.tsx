...
import { SEGMENT_LABELS, SEGMENT_ORDER, effectiveSegment, occupiesSegment } from '../utils/segment'
...
export function InspectionsPage() {
  ...
  const selectedBatchId = Form.useWatch('productionBatchId', createForm) as number | undefined
  ...
  // 同一批次同一段位最多保留一条待完成或已合格样本；已占用段位在选择器中禁用，
  // 后端仍会在事务内强校验并发提交。
  const occupiedSegments = useMemo(() => {
    const target = batches.find((batch) => batch.id === selectedBatchId)
    const occupied = new Set<SampleSegment>()
    for (const sample of target?.inspections || []) {
      const segment = effectiveSegment(sample)
      if (segment && occupiesSegment(sample)) occupied.add(segment)
    }
    return occupied
  ...
  const columns: ColumnsType<InspectionSample> = [
    ...
    { title: '段位', render: (_, row) => <SegmentTag sample={row} short /> },
  ...
  return (
    <div className="page-stack">
      ...
      <Modal title="登记检验样本" width={640} open={createOpen} confirmLoading={saving} onOk={() => void create()} onCancel={() => setCreateOpen(false)} okText="登记" cancelText="取消">
        <Form form={createForm} layout="vertical" onValuesChange={(changed) => { if (changed.productionBatchId) createForm.setFieldValue('segment', undefined) }}><Form.Item name="productionBatchId" label="生产批次" rules={[{ required: true }]}><Select showSearch optionFilterProp="label" options={batches.map((batch) => ({ value: batch.id, label: `${batch.batchNo} · ${batch.specification}` }))} /></Form.Item><Row gutter={16}><Col span={12}><Form.Item name="sampleCode" label="样本编号" rules={[{ required: true, min: 3 }]}><Input /></Form.Item></Col><Col span={12}><Form.Item name="segment" label="抽样段位" rules={[{ required: true, message: '请选择批次起始段、中段或末段' }]}><Radio.Group optionType="button" buttonStyle="solid" options={SEGMENT_ORDER.map((segment) => ({ label: SEGMENT_LABELS[segment], value: segment, disabled: occupiedSegments.has(segment) }))} /></Form.Item></Col></Row><Form.Item label="抽样位置（留空自动使用段位名称）" name="samplingPosition"><Input maxLength={120} placeholder="例如：批次起始段 1 号工位" /></Form.Item><Form.Item name="inspectionItem" label="检验项目" rules={[{ required: true }]}><Input placeholder="密封强度、染色渗透等" /></Form.Item><Form.Item name="acceptanceRange" label="接受范围" rules={[{ required: true }]}><Input placeholder="≥ 1.5 N/15mm" /></Form.Item><Form.Item name="notes" label="备注"><Input.TextArea rows={2} /></Form.Item></Form>
      </Modal>
